...
class RoundRobinPairingTest extends TestCase
{
    ...
    public function test_correct_number_of_rounds()
    {
    ...
    public function test_every_player_plays_against_every_other_player_once()
    {
        for ($i = static::MIN_PLAYERS; $i <= static::MAX_PLAYERS; $i++) {
            $pairings = $this->getPairings($i);
                ...
                foreach ($this->getPlayers($i) as $player2) {
                    ...
                    foreach ($pairings as $round) {
                        ...
                        foreach ($groups as $group) {
                            if (! in_array($player1, $group)) {
                                continue;
                            }
                            if (! in_array($player2, $group)) {
                                continue;
                            }
                            $matchCount++;
                        }
    ...
    public function test_every_player_plays_against_every_other_player_twice_if_double()
    {
        for ($i = static::MIN_PLAYERS; $i <= static::MAX_PLAYERS; $i++) {
    ...
    public function test_balanced_player_order()
    {
        for ($i = static::MIN_PLAYERS; $i <= static::MAX_PLAYERS; $i++) {
            $pairings = $this->getPairings($i);
    ...
    public function test_balanced_player_order_against_same_opponent_if_double()
    {
        for ($i = static::MIN_PLAYERS; $i <= static::MAX_PLAYERS; $i++) {
            $pairings = $this->getPairings($i, true);
            foreach ($pairings as $round) {
