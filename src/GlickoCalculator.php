...
class GlickoCalculator
{
     ...
     * @return float
     */
    public static function E($rating, $opponent_rating, $opponent_rd)
    {
     ...
     * @return float
     */
    public static function expectedOutcome($rating, $rd, $opponent_rating, $opponent_rd)
    {
     ...
     *
     * @param  int  $default_rating
     * @param  int  $default_rd
     * @param  float  $c
     * @param  int  $rd_min
     * @return void
     */
        ...
        if (count($result) === 2) {
            ...
            foreach ($result as $i => $player) {
                foreach ($result as $j => $current_player) {
                    ...
                    if (in_array($current_player, $handled_players)) {
                        continue;
                    }
     ...
     *
     * @param  float  $rd
     * @param  int  $periods  - Number of times to advance
     * @return float
     */
    public function advanceRD($rd, $periods = 1)
    {
        for ($i = 0; $i < $periods; $i++) {
            if ($rd >= $this->default_rd) {
                return $this->default_rd;
            }
