...
class GolfHole
{
	...
	public function getStrokesForScore($score) {
		if (!$score) return 10;
		if ($score >= $this->targetScore) return 0;

		$brackets = $this->getScoreBrackets();

		if ($score >= $brackets[6][0] && $score <= $brackets[6][1]) return 6;
		if ($score >= $brackets[7][0] && $score <= $brackets[7][1]) return 7;
		if ($score >= $brackets[8][0] && $score <= $brackets[8][1]) return 8;
		if ($score >= $brackets[9][0] && $score <= $brackets[9][1]) return 9;
