    ...
    <div class="container">
      ...
      <form action="/" method="get" class="form-inline">
          ...
          <select name="rounds" class="form-control">
            ...
            <option value="3" <?php echo ! empty($_GET['rounds']) && (int) $_GET['rounds'] === 3 ? 'selected' : '' ?>>3 rounds</option>
            <option value="4" <?php echo ! empty($_GET['rounds']) && (int) $_GET['rounds'] === 4 ? 'selected' : '' ?>>4 rounds</option>
            <option value="5" <?php echo ! empty($_GET['rounds']) && (int) $_GET['rounds'] === 5 ? 'selected' : '' ?>>5 rounds</option>
            <option value="10" <?php echo ! empty($_GET['rounds']) && (int) $_GET['rounds'] === 10 ? 'selected' : '' ?>>10 rounds</option>
            <option value="12" <?php echo ! empty($_GET['rounds']) && (int) $_GET['rounds'] === 12 ? 'selected' : '' ?>>12 rounds</option>
            <option value="13" <?php echo ! empty($_GET['rounds']) && (int) $_GET['rounds'] === 13 ? 'selected' : '' ?>>13 rounds</option>
          </select>
      ...
      require '../../src/GroupTieredSwissPairing.php';
            ...
            if (! empty($_GET['player_count']) && is_numeric($_GET['player_count']) && ! empty($_GET['rounds']) && is_numeric($_GET['rounds'])) {
                ...
                echo "<hr><h2>Tier sizes for {$player_count} players (using tiers for {$group_map['key']} players)</h2>";
                $round = 0;
                ...
                while ($round < $groupBuilder->rounds) {
                    ...
                    $id = $round + 1;
                    echo "<tr><th style='width:150px;'>Round {$id}</th><td>";
                    foreach ($map as $m) {
                        echo "<div class='text-center' style='box-sizing:border-box;border-left:2px solid white;border-right:2px solid white;background:#ccc;display:inline-block;width:".($unit * $m)."%;'>{$m}</div>";
                    }
                ...
                echo "<h2>Groups for {$player_count} players</h2>";
